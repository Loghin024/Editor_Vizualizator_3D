...
void figuresInfo::componentsNames(sf::RenderWindow& window, std::string Name, int pozMx, int pozMy, bool changeUp, bool changeDown, int& x, int& y, int language, Camera camera, int& compScroll)
{
	...
	{
		if (copieScroll > (numberOfComponents - 13)) copieScroll--;
	}
			...
			if (copieScroll <= 0)
			{
				if (maximComponents2 < 14)
				{
					...
					if (pozMx > 1390 && pozMy > 50 + 30 * line + 5 + 2 && pozMy < 50 + 30 * line + 5 + 2 + 30)
					{
						...
						figureInfoPosition(window, "pozitie:", Name, sir, changeUp, changeDown, x, y, camera);
						figureInfoSize(window, "size", Name, sir, 1, changeUp, changeDown, x, y, camera);
						figureInfoType(window, "tip:", Name, sir, language);
					}
					else
						components.setButtonColor(81, 81, 81, 255);
...
void figuresInfo::scrollBar(sf::RenderWindow& window, int numberOfComponents, int howMuch, int& scrollBar)
{
	...
	{
		scroll.setSize(Vector2f(15, 30 * 13));
		int resize = scroll.getSize().y - ((numberOfComponents - 14) *5);
		scroll.setSize(Vector2f(15, resize));
		if (scrollBar > numberOfComponents - 15) scrollBar = (numberOfComponents - 15);
...
void figuresInfo::figureInfoType(sf::RenderWindow& window, std::string Type, std::string Name, std::string componentName, int language)
{
	int index = 0;
	using namespace sf;
	Vector2u size = window.getSize();
...
void figuresInfo::figureInfoPosition(sf::RenderWindow& window, std::string position, std::string Name, std::string componentName, bool& changeUp, bool& changeDown, int& pozMx, int& pozMy,  Camera camera)
{
	...
	if (pozMx > window.getSize().x - 220 + l.width && pozMx < window.getSize().x - 120 + l.width && pozMy > 548 && pozMy < 548 + 24)
	{
		...
		if (changeDown) { x--; change(window, camera, Name); changeDown = 0; }
		if (changeUp) {
			x++;change(window, camera, Name);
			changeUp = 0;
	}
	}
	else
...
void figuresInfo::figureInfoSize(sf::RenderWindow& window, std::string _size, std::string Name, std::string componentName, int language, bool changeUp, bool changeDown, int pozMx, int pozMy, Camera camera)
{
	int index = 0;
	using namespace sf;
	using namespace std;
	...
	if (pozMx > window.getSize().x - 220 + textWidht.width && pozMx < window.getSize().x - 120 + textWidht.width && pozMy > 640 && pozMy < 640 + 24)
	{
		...
		if (changeDown){ h--; change(window, camera, Name);
	}
		if (changeUp){ h++;change(window, camera, Name);
	}
	}
	else
	...
	if (pozMx > window.getSize().x - 220 + textWidht.width && pozMx < window.getSize().x - 120 + textWidht.width && pozMy > 670 && pozMy < 670 + 24)
	{
		...
		if (changeDown){ l--;change(window, camera, Name);
	}
		if (changeUp) {l++; change(window, camera, Name);
	}
	}
	else
...
void figuresInfo::change(sf::RenderWindow& window, Camera camera, std::string saveNameOfProject)
{
	window.clear();
